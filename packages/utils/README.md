...
vp test          # run tests
vp build         # regenerate schema then build (runs: bebopc build && vp pack)
```

> `src/bebop.ts` is auto-generated by `bebopc` from the `.bop` schema files. Do not edit it by hand. Run `bebopc build` (or `vp build`) to regenerate after changing `schema/*.bop`.
...
bebopc build
# or
vp build  # runs bebopc build then packs the library
```
