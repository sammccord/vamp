...
// functions) also imports `@vampgg/worker` to define Durable Object classes, and
// the real `@vampgg/worker` imports the CF-runtime-only `cloudflare:workers` virtual
// module. Intercept the `@vampgg/worker` import (made by the vite-transformed
  ...
  plugins: [
    {
      ...
      resolveId(source: string) {
        if (source === "@vampgg/worker") return workerStub;
        return null;
      },
    },
    solid(),
  ...
  test: {
    ...
    server: {
      deps: {
        ...
        // in bebop@3.2.3 — inline so Vite transforms with lazy bindings. @vampgg/solid
        // is inlined so its solid-js import dedupes to the one instance.
        inline: [/@tempojs\//, "@vampgg/solid"],
      },
    },
  },
});
